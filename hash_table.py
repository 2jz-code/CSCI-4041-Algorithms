...
def database_insert(database, robot):
    ...
    hash_name = hash_function(robot.name)
    database.name_table[hash_name].append(robot)

    hash_shape = hash_function(robot.shape)
    database.shape_table[hash_shape].append(robot)

    hash_color = hash_function(robot.color)
    database.color_table[hash_color].append(robot)
...
def database_search(database,field,value):
    if field == 'name':
        ...
        index = 0
    if field == 'shape':
        table = database.shape_table
        index = 1
    if field == 'color':
        table = database.color_table
        index = 2
...
def database_delete(database,field,value):
    ...
    i = 0
    while i < len(slot):
        if slot[i].data[index] == value:
            removed_bots.append(slot.pop(i))
        else:
            i += 1

    for robot in removed_bots:
        if robot in database.name_table[hash_function(robot.data[0])]:
            database.name_table[hash_function(robot.data[0])].remove(robot)
        if robot in database.shape_table[hash_function(robot.data[1])]:
            database.shape_table[hash_function(robot.data[1])].remove(robot)
        if robot in database.color_table[hash_function(robot.data[2])]:
            database.color_table[hash_function(robot.data[2])].remove(robot)
