...
def Max_heapify(task_heap, i):
  ...
  largest = i
  if L < len(task_heap):
    if L >= i and task_heap[L].priority > task_heap[i].priority: 
        largest = L 
    else:
        largest = i
  if R < len(task_heap):
    if R >= i and task_heap[R].priority > task_heap[largest].priority:
        largest = R
  if largest != i:
    task_heap[i], task_heap[largest] = task_heap[largest], task_heap[i]
...
def extract_max_priority_task(task_heap):
    if len(task_heap) < 1:
        return None
    heap_size = len(task_heap) - 1
    max = task_heap[0]
    task_heap[0] = task_heap[heap_size]
    task_heap.pop()
    Max_heapify(task_heap, 0)
    return max
...
def increase_task_priority(task_heap,i,new_priority):
    if i >= len(task_heap) or new_priority < task_heap[i].priority:
        return None
    task_heap[i].priority = new_priority
    parent = (i-1) // 2
    while i > 0 and task_heap[parent].priority < task_heap[i].priority:
        task_heap[i], task_heap[parent] = task_heap[parent], task_heap[i]
        i = parent
        parent = (i-1) // 2
